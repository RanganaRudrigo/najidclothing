...
function bindState() {
	$.ajax({
		url:  ZONEAPI+"getZoneByCountryId",
		async: true,
		cache: false,
		dataType: "json",
		data: {
		...
		success: function (jsonData) {
			var state = $('#id_state');
			state.html("");
			$(jsonData).each(function(key, item){
				$('#id_state').append('<option value="' + parseInt(item.zone_id) + '">' + item.name + '</option>');
			});
			state.val(state.data('id'));
			state.uniform();
		}
	});
}