...
$(document).ready(function(){
	$('.cart_quantity_up').off('click').on('click', function(e){
		e.preventDefault();
		upQuantity(
			$(this).attr('id').replace('cart_quantity_up_', '') ,
			parseInt( $("#quantity_"+$(this).attr('id').replace('cart_quantity_up_', '')).val() )
		);
	});
	$('.cart_quantity_down').off('click').on('click', function(e){
		...
		downQuantity(
			...
			parseInt( $("#quantity_"+$(this).attr('id').replace('cart_quantity_down_', '')).val() )
		);
	});
...
function updateCartSummaryNew(json){
	var i;
	var nbrProducts = 0;
	...
	product_list = json.products ;

	for (i in product_list) {
		var rowid = product_list[i].rowid ;
		$("#quantity_"+rowid).val(product_list[i].quantity);
		$("#total_product_price_"+rowid).html( (product_list[i].quantity * product_list[i].price_float).toFixed(2) + " QAR"   );

	}
	$("#total_price").html(json.total + " QAR"  );
}
