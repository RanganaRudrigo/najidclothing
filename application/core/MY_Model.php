...
class MY_Model extends CI_Model
{
    ...
    protected function relation_tables(){
        $tables = func_get_args();
        if(!empty($this->join) && !empty($tables) ){
            foreach ($this->join as $data){
                if( ( is_array($tables) && in_array($data[0],$tables) ) || $data[0] == $tables )
                    $this->_database->join($data[0],$data[1])->select($data[2],FALSE);
            }
        }
    }
