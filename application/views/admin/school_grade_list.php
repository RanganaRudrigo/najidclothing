...
<div id="main_wrapper">
    <div class="page_bar clearfix">
        <div class="row">
            <div class="col-md-8">
                <h1 class="page_title">  <?= ucfirst($this->controller) ?> - Grade  </h1>
                <p class="text-muted"> </p>
            </div>
            <div class="col-md-4 text-right">
                <a href="<?= base_url() ?>admin/<?= $this->controller ?>/grade/create" class="btn btn-primary"><i class="fa fa-plus"></i> Create </a>
            </div>
        </div>
    </div>
                        ...
                        <div class="panel-body table-responsive">
                            <table id="dt_tableTools" class=" table table-striped table-bordered ">
                                ...
                                <?php if (is_array($records)) foreach ($records as $k => $row): ?>
                                    <tr>
                                        <td><?= $k + 1 ?></td>
                                        <td> <a href="<?= current_url() ?>/view/<?= $row->school_id ?>" ><?= $row->school_title ?></a> </td>
                                        <td><?= $row->title ?></td>
                                        <td><?= $row->code ?></td>
                                        <td>
